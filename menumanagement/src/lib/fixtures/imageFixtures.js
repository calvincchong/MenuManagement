export const imageFixtures = {
  sampleImageObjects: [
    {
      name: 'General Apps',
      catItemNum: 'A',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1678385134/Koo%20Koo%20Chicken/kkc-top-down-menu-item_seijj1.jpg',
    },
    {
      name: 'Lobak',
      catItemNum: 'A12',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Lobak-min_ph9ovw.jpg',
    },
    {
      name: 'Ong Choy',
      catItemNum: 'R14',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918349/okra-min_cm9k8v.jpg',
    },
    {
      name: 'Ong Choy',
      catItemNum: 'R14',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/2F4A0034-min_ncp3nx.jpg',
    },
    {
      name: 'Spices',
      catItemNum: 'XS1',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Koo_Koo_Restaurant_Spice-min_p3qskx.jpg',
    },
  ],
  'Stir-Fry Noodles': [
    {
      name: 'Pearl Noodles',
      catItemNum: 'A',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934000/PearlNoodleStirFry_csw0fr.jpg',
    },
  ],
  Appetizers: [
    {
      name: 'Lobak',
      catItemNum: 'A12',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Lobak-min_ph9ovw.jpg',
    },
    {
      name: 'Satay',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934112/SatayYinYang_vcama9.jpg',
    },
    {
      name: 'Shrimp Papaya Salad',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934112/ShrimpSaladAppetizer_p1ptnw.jpg',
    },
  ],
  'Soups To Share': [
    {
      name: 'Fishhead Preserved Vegetables',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934123/FishHeadNoodleSoupWithPreservedVegetable2_sciiem.jpg',
    },
  ],
  'Rice Dishes': [
    {
      name: 'Nasi Lemak',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934115/AM9I9941-min_cfa1vr.jpg',
    },
  ],
  Meat: [
    {
      name: 'Curry Beef',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934112/Curry_Beef_on_Sizzling_Plate-min_dc6d0j.jpg',
    },
  ],
  Sides: [
    {
      name: 'Spices',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Koo_Koo_Restaurant_Spice-min_p3qskx.jpg',
    },
  ],
  Vegetable: [
    {
      name: 'Okra w. Belachan',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918349/okra-min_cm9k8v.jpg',
    },
    {
      name: 'Ong Choy',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934116/AM9I9957-min_xyvmxz.jpg',
    },
  ],
  Specialty: [
    {
      name: 'Prawn',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934119/AM9I9999-min_r1xcna.jpg',
    },
  ],
  'Soup Noodles': [
    {
      name: 'Fishhead Preserved Vegetables',
      url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934123/FishHeadNoodleSoupWithPreservedVegetable2_sciiem.jpg',
    },
  ],
  Poultry: [],
  'Drinks & Desserts': [],
};

// const sampleImages = [
//   'https://res.cloudinary.com/dq6rqplja/image/upload/v1678385134/Koo%20Koo%20Chicken/kkc-top-down-menu-item_seijj1.jpg',
//   'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Lobak-min_ph9ovw.jpg',
//   'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918349/okra-min_cm9k8v.jpg',
//   'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/2F4A0034-min_ncp3nx.jpg',
//   'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Koo_Koo_Restaurant_Spice-min_p3qskx.jpg',
// ];

// const sampleImageObjects = [
//   {
//     name: 'General Apps',
//     catItemNum: 'A',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1678385134/Koo%20Koo%20Chicken/kkc-top-down-menu-item_seijj1.jpg',
//   },
//   {
//     name: 'Lobak',
//     catItemNum: 'A12',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Lobak-min_ph9ovw.jpg',
//   },
//   {
//     name: 'Ong Choy',
//     catItemNum: 'R14',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918349/okra-min_cm9k8v.jpg',
//   },
//   {
//     name: 'Ong Choy',
//     catItemNum: 'R14',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/2F4A0034-min_ncp3nx.jpg',
//   },
//   {
//     name: 'Spices',
//     catItemNum: 'XS1',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Koo_Koo_Restaurant_Spice-min_p3qskx.jpg',
//   },
// ];

// const stirFryNoodleImages = [
//   {
//     name: 'Pearl Noodles',
//     catItemNum: 'A',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934000/PearlNoodleStirFry_csw0fr.jpg',
//   },
// ];

// const appetizersImages = [
//   {
//     name: 'Lobak',
//     catItemNum: 'A12',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Lobak-min_ph9ovw.jpg',
//   },
//   {
//     name: 'Satay',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934112/SatayYinYang_vcama9.jpg',
//   },
// ];

// const soupsImages = [
//   {
//     name: 'Fishhead Preserved Vegetables',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934123/FishHeadNoodleSoupWithPreservedVegetable2_sciiem.jpg',
//   },
// ];

// const riceDishes = [
//   {
//     name: 'Nasi Lemak',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934115/AM9I9941-min_cfa1vr.jpg',
//   },
// ];

// const meatImages = [
//   {
//     name: 'Curry Beef',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681934112/Curry_Beef_on_Sizzling_Plate-min_dc6d0j.jpg',
//   },
// ];

// const spiceImages = [
//   {
//     name: 'Spices',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918350/Koo_Koo_Restaurant_Spice-min_p3qskx.jpg',
//   },
// ];

// const vegetableImages = [
//   {
//     name: 'Ong Choy',
//     url: 'https://res.cloudinary.com/dq6rqplja/image/upload/v1681918349/okra-min_cm9k8v.jpg',
//   },
// ];
