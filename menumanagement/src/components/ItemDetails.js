  ...
  return (
    <div className={styles.modalContainer}>
      <div className={styles.detailsContainer}>
        ...
        <AiOutlineCloseCircle className={styles.closeButton} onClick={() => {dispatch(selectItem(false))}} />
        <div className={styles.imageContainer}>
          <Image
          src='https://rasamalaysia.com/wp-content/uploads/2019/04/chicken-satay3.jpg'
          alt={item !== null ? item.value.menuName : 'Koo Koo Chicken Menu Item'}
          fill
          style={{objectFit: 'cover'}}
          />
        </div>
        <div className={styles.itemHeader}>{item !== null ? item.value.menuName : 'Salt & Pepper Squid 椒鹽鮮魷'}</div>
        <div className=''>{item !== null ? item.value.description : ''}</div>
        {/* <div className=''>{item['options'] !== undefined ? item.options : '' }</div> */}

        <button className={styles.addToCartButton}> Add {item.quantity} to Cart </button>
      </div>
    </div>
  )
